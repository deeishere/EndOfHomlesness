...
HardwareSerial r307Serial(2); // UART2: GPIO16=RX, GPIO17=TX
Adafruit_Fingerprint finger = Adafruit_Fingerprint(&r307Serial);
...
enum FingerState { STATE_DETECT, STATE_ENROLL, STATE_DELETE };
volatile FingerState currentState = STATE_DETECT;
volatile int deleteId = -1;
...
void setup() {
  //////////////
  NimBLEDevice::init("ESP32-Fingerprint-BLE");
  pServer = NimBLEDevice::createServer();
  NimBLEService* uartService = pServer->createService(SERVICE_UUID);

  NimBLECharacteristic* rxCharacteristic = uartService->createCharacteristic(
  CHARACTERISTIC_RX, NIMBLE_PROPERTY::WRITE);
  txCharacteristic = uartService->createCharacteristic(
  CHARACTERISTIC_TX, NIMBLE_PROPERTY::NOTIFY);
  ...
  switch (currentState) {
    ...
    case STATE_ENROLL:
      ...
      if (getFingerprintEnroll() == true) {
        sendBLE("Enroll success");
        id++;
      } else {
        sendBLE("Enroll failed");
      }
    ...
    case STATE_DELETE:
      if (deleteId > 0) {
        ...
        if (deleteFingerprint(deleteId) == FINGERPRINT_OK) {
          sendBLE("Delete success");
        } else {
          sendBLE("Delete failed");
        }
        deleteId = -1;
      }
      currentState = STATE_DETECT;
